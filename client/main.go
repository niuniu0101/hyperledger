...
type PipelineConfig struct {
	...
	FlushTimeoutUs int   `json:"flush_timeout_us"`
}
...
func loadPipelineConfig(path string) PipelineConfig {
	...
	if tmp.FlushTimeoutUs > 0 {
		cfg.FlushTimeoutUs = tmp.FlushTimeoutUs
	}
	return cfg
}
...
func main() {
	...
	case 2:
		...
		mode.RunPipelineMode(hrm, serverClients, files, ringToServer, centerServer, workerCount, verifyProof, cfg.CacheBytes, time.Duration(cfg.FlushTimeoutUs)*time.Microsecond)
