...
var serverConfigs = map[uint8][]string{
	0: {"8.208.81.144:8082"},  // 环0: node0,node4,node8...node36
	1: {"47.237.16.182:8082"}, // 环1: node1,node5,node9...node37
	2: {"8.220.202.165:8082"}, // 环2: node2,node6,node10...node38
	3: {"47.251.71.236:8082"}, // 环3: node3,node7,node11...node39
...
func (cm *ChunkManager) collectInactiveDataChunks() []*ChunkMeta {
	...
	for ringIndex := 0; ringIndex < NumDataRings; ringIndex++ {
		...
		for _, chunk := range cm.chunkIndex {
			if uint8(ringIndex) == chunk.RingIndex && !chunk.IsActive {
				...
				/* 				if !cm.shouldStoreLocally(chunk) {
					delete(cm.chunkIndex, chunk.ChunkID) // 从索引中删除，防止重复使用
				} */
		...
		if !found {
			log.Printf("No available inactive chunk found for ring %d", ringIndex)
		}
	}
	return chunks
}
...
func (cm *ChunkManager) sendSingleFileWithConnection(conn net.Conn, chunk *ChunkMeta, objectKey string, chunkContent []byte) error {
	...
	log.Printf("Object meta found: offset=%d, length=%d", objectMeta.Offset, objectMeta.Length)

	// 从chunk内容中提取该对象的数据
	start := int(objectMeta.Offset)
	end := start + int(objectMeta.Length)
	log.Printf("Extracting data from chunk: start=%d, end=%d, chunkSize=%d", start, end, len(chunkContent))

	if end > len(chunkContent) {
		return fmt.Errorf("object data out of chunk bounds: key=%s, offset=%d, length=%d, chunkSize=%d",
			objectKey, objectMeta.Offset, objectMeta.Length, len(chunkContent))
	}
	objectData := chunkContent[start:end]

	log.Printf("Preparing to send file %s, size: %d bytes", objectKey, len(objectData))

	// 准备上传报文
	log.Printf("Preparing upload message")
	message := cm.prepareFileUploadMessage(objectKey, chunk.NodeName, objectData)
	log.Printf("Message prepared, total size: %d bytes", len(message))

	// 发送报文
	log.Printf("Starting to write message to connection")
	startTime := time.Now()
	n, err := conn.Write(message)
	if err != nil {
		return fmt.Errorf("send file data to server: %v", err)
	}

	log.Printf("Successfully sent %d bytes to server in %v", n, time.Since(startTime))
	...
	if _, err := conn.Read(response); err != nil {
		log.Printf("No response from server (might be expected): %v", err)
	} else {
		log.Printf("Received response from server: %v", response[0])
	}

	log.Printf("File %s transfer completed", objectKey)
	return nil
}
